...
backend API. It exposes **45 MCP tools** covering 7 resource domains.
...
- **Full Presenton Coverage** — 45 tools mapped to Presenton API endpoints
...
- **Comprehensive Testing** — Automated tests covering all tool domains,
...
|----------|----------|-------------|
| `PRESENTON_BASE_URL` | Yes | Docker-internal URL of the Presenton API (e.g. `http://presenton-api:80`). |
...
3. Run the server:
    ```bash
    export PRESENTON_BASE_URL=http://localhost:7531
    export MCP_SERVER_PORT=5641
    python -m src.main
    ```
...
```bash
docker build -t presenton-mcp .
docker run -d --name presenton-mcp \
    -e PRESENTON_BASE_URL="http://presenton-api:80" \
    -e MCP_SERVER_PORT=5641 \
    -e IS_STATEFUL=false \
    presenton-mcp
```

The MCP server serves at `http://presenton-mcp:5641/mcp` (Streamable HTTP).
...
- **Auth** — The MCP server accepts `Bearer` tokens from clients and
  forwards them as `Basic` auth to the Presenton backend.

## API Tool Mapping

The server implements 45 MCP tools organized into the following categories:

### Presentation Management (11 tools)
